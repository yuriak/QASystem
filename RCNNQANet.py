...
class RCNNQANet():
    def __init__(self, pretrained_embedding, encoder_units_number=[300, 100], attention_size=[100], cnn_filters=[300, 200, 100], context_kernal_size=3, question_kernel_size=2, learning_rate=0.001, log_dir='./logs', model_path='./RNNQANet'):
        ...
        with tf.variable_scope('embedding', initializer=tf.contrib.layers.xavier_initializer()):
            W = tf.Variable(pretrained_embedding, trainable=True, dtype=tf.float32)
    ...
    def gated_attention(self, inputs, memory, hidden, keep_prob=1.0, scope="dot_attention"):
